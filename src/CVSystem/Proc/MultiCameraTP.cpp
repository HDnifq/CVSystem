...
///-------------------------------------------------------------------------------------------------
struct MultiCameraTP::Impl
{

};

MultiCameraTP::MultiCameraTP()
{
}
...
MultiCameraTP* MultiCameraTP::m_pInstance = NULL;

/// <summary> 提供给BaseThread的init执行事件. </summary>
void MultiCameraTP::init(std::shared_ptr<BaseThread>& tb)
{
...
void MultiCameraTP::release(std::shared_ptr<BaseThread>& tb)
{
...
void MultiCameraTP::workonce(std::shared_ptr<BaseThread>& tb)
{
...
bool MultiCameraTP::openCamera(uint activeIndex)
{
    ...
    if (_cameraGrab.open()) {
        ...
        //综合分析计算线程
        this->_thread = BaseThread::creat(std::bind(&MultiCameraTP::init, this, std::placeholders::_1),
                                          std::bind(&MultiCameraTP::workonce, this, std::placeholders::_1),
                                          std::bind(&MultiCameraTP::release, this, std::placeholders::_1));
...
void MultiCameraTP::close(bool isDeleteProc)
{
...
void MultiCameraTP::addProc(const pFrameProc& proc)
{
    this->vProc.push_back(proc);
}

void MultiCameraTP::addProc(FrameProc* proc)
{
    this->vProc.push_back(pFrameProc(proc));
}

void MultiCameraTP::setActiveProc(uint index)
{
